...
function flushAndValidatePlugins(newState, oldState, pluginOpts) {
    newState = turnOrder.Flush(newState, pluginOpts);
    const isInvalid = turnOrder.IsInvalid(newState, pluginOpts);
    if (!isInvalid)
        return [newState];
    const { plugin, message } = isInvalid;
    turnOrder.error(`plugin declared action invalid: ${plugin} - ${message}`);
    return [
        newState,
